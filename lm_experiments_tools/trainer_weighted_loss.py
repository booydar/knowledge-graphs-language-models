    ...
    def step(self, batch, is_train_mode=True) -> Tuple[Dict[str, float], Dict[str, list]]:
        ...
        with torch.set_grad_enabled(is_train_mode):
            for j in range(0, len(batch['input_ids']), batch_size):
                ...
                filtered_subbatch = dict(**{k: subbatch[k] for k in subbatch if k in self.model_forward_args})
                labels = filtered_subbatch.pop('labels')
                decoder_input_ids = self.model._shift_right(labels)
                outputs = self.model(**filtered_subbatch, decoder_input_ids=decoder_input_ids)
                    
                sep2_id, entity_token_weight = self.model.entity_sep_id, self.model.entity_tokens_weight

                lm_logits = outputs['logits']
                loss_fct = CrossEntropyLoss(ignore_index=-100, reduction='none')
                loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1))

                entity_mask = torch.ones_like(labels).float()
                for row, start_ind in torch.nonzero(labels == sep2_id):
                    entity_mask[row, :start_ind] *= entity_token_weight
                flat_entity_mask = entity_mask.view(-1)

                loss = (loss * flat_entity_mask).mean()
                outputs['loss'] = loss
